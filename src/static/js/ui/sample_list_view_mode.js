/**
 * Sample List - View Mode Switcher
...
HopaoUI.register('sample_list_view_mode', () => {
    const root = document.querySelector('[data-hook="sample-list-root"]');
    if (!root) return;

    const cardsPanel = root.querySelector('[data-hook="sample-cards-panel"]');
    const tablePanel = root.querySelector('[data-hook="sample-table-panel"]');
    ...
    document.addEventListener('click', (e) => {
        const toggleBtn = e.target.closest('[data-action="sample-view-toggle"]');
        if (!toggleBtn) return;
